...
public class StreamDecoder extends Reader
{
    ...
    String encodingName() {
        /* J2ObjC: There are no instances of HistoricallyNamedCharset.
    ...
    // J2ObjC: Avoid link-time dependency on FileChannel class.
    private static native boolean isFileChannel(ReadableByteChannel ch) /*-[
        static Class fileChannelCls;
        static dispatch_once_t once;
        dispatch_once(&once, ^{ fileChannelCls = objc_getClass("JavaNioChannelsFileChannel"); });
        return [ch isKindOfClass:fileChannelCls];
    ]-*/;

    private boolean inReady() {
        try {
        return (((in != null) && (in.available() > 0))
                || isFileChannel(ch)); // ## RBC.available()?
        } catch (IOException x) {
        return false;
        }
    }
