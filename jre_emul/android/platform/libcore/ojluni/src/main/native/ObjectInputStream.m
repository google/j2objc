                                             ...
                                             jint nfloats)
{
    ...
    bytes = (*env)->GetPrimitiveArrayCritical(env, (jarray)src, NULL);
    if (bytes == NULL)          /* exception thrown */
        return;

    /* fetch dest array */
    if (dst == NULL) {
        (*env)->ReleasePrimitiveArrayCritical(env, (jarray)src, bytes, JNI_ABORT);
        JNU_ThrowNullPointerException(env, NULL);
        return;
    }
    floats = (*env)->GetPrimitiveArrayCritical(env, (jarray)dst, NULL);
    ...
    (*env)->ReleasePrimitiveArrayCritical(env, (jarray)src, bytes, JNI_ABORT);
    (*env)->ReleasePrimitiveArrayCritical(env, (jarray)dst, floats, 0);
}
...

{
    ...
    doubles = (*env)->GetPrimitiveArrayCritical(env, (jarray)dst, NULL);
    ...
    (*env)->ReleasePrimitiveArrayCritical(env, (jarray)src, bytes, JNI_ABORT);
    (*env)->ReleasePrimitiveArrayCritical(env, (jarray)dst, doubles, 0);
}
