                                                  ...
                                                  jstring abspath)
{
    ...
    // ----- END android -----
        ...
        if (statMode(path, &mode)) {
            int fmt = mode & S_IFMT;
            rv = (jint) (JavaIoFileSystem_BA_EXISTS
                  | ((fmt == S_IFREG) ? JavaIoFileSystem_BA_REGULAR : 0)
                  | ((fmt == S_IFDIR) ? JavaIoFileSystem_BA_DIRECTORY : 0));
        }
    } END_PLATFORM_STRING(env, path);
    return rv;
}
                                           ...
                                           jboolean owneronly)
{
    ...
    case JavaIoFileSystem_ACCESS_READ:
        ...
        break;
    case JavaIoFileSystem_ACCESS_WRITE:
        if (owneronly)
            amode = S_IWUSR;
        else
            amode = S_IWUSR | S_IWGRP | S_IWOTH;
        break;
    case JavaIoFileSystem_ACCESS_EXECUTE:
        if (owneronly)
            amode = S_IXUSR;
        else
                                                 ...
                                                 jobject file)
{
    ...
    // WITH_FIELD_PLATFORM_STRING(env, file, ids.path, path) {
    struct stat64 sb;
    if (stat64([(JavaIoFile *)file getPath].UTF8String, &sb) == 0) {
                                  ...
                                  jobject file)
{
    ...
    // WITH_FIELD_PLATFORM_STRING(env, file, ids.path, path) {
        dir = opendir([(JavaIoFile *)file getPath].UTF8String);
    // } END_PLATFORM_STRING(env, path);
                                      ...
                                      jobject file, jint t)
{
    ...
    if (statvfs64([(JavaIoFile *)file getPath].UTF8String, &fsstat) == 0) {
        switch(t) {
        case JavaIoFileSystem_SPACE_TOTAL:
            rv = jlong_mul(long_to_jlong(fsstat.f_frsize),
                           long_to_jlong(fsstat.f_blocks));
            break;
        case JavaIoFileSystem_SPACE_FREE:
            rv = jlong_mul(long_to_jlong(fsstat.f_frsize),
                           long_to_jlong(fsstat.f_bfree));
            break;
        case JavaIoFileSystem_SPACE_USABLE:
            rv = jlong_mul(long_to_jlong(fsstat.f_frsize),
