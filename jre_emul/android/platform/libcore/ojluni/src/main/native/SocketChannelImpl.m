...
JNIEXPORT jint JNICALL Java_sun_nio_ch_SocketChannelImpl_checkConnect(JNIEnv* env, jobject this,
                                                                      jobject fdo, bool block,
                                                                      bool ready) {
  int error = 0;
  ...
  if (!ready) {
    ...
    if (!block && (result == 0)) return IOS_UNAVAILABLE;
  }
...
Java_sun_nio_ch_SocketChannelImpl_sendOutOfBandData(JNIEnv* env, jclass this,
                                                    jobject fdo, jbyte b)
{
    int n = (int)send(fdval(env, fdo), (const void*)&b, 1, MSG_OOB);
