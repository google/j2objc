                                              ...
                                              jint nfloats)
{
    ...
    floats = (*env)->GetPrimitiveArrayCritical(env, (jarray)src, NULL);
    if (floats == NULL)         /* exception thrown */
        return;

    /* fetch dest array */
    if (dst == NULL) {
        (*env)->ReleasePrimitiveArrayCritical(env, (jarray)src, floats, JNI_ABORT);
        JNU_ThrowNullPointerException(env, NULL);
        return;
    }
    bytes = (*env)->GetPrimitiveArrayCritical(env, (jarray)dst, NULL);
    ...
    (*env)->ReleasePrimitiveArrayCritical(env, (jarray)src, floats, JNI_ABORT);
    (*env)->ReleasePrimitiveArrayCritical(env, (jarray)dst, bytes, 0);
}
                                               ...
                                               jint ndoubles)
{
    ...
    doubles = (*env)->GetPrimitiveArrayCritical(env, (jarray)src, NULL);
    if (doubles == NULL)                /* exception thrown */
        return;

    /* fetch dest array */
    if (dst == NULL) {
        (*env)->ReleasePrimitiveArrayCritical(env, (jarray)src, doubles, JNI_ABORT);
        JNU_ThrowNullPointerException(env, NULL);
        return;
    }
    bytes = (*env)->GetPrimitiveArrayCritical(env, (jarray)dst, NULL);
