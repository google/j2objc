...
// Check that method has a type encoding with only objects and primitive types, not structs or
// other types not generated by j2objc.
static BOOL HasSupportedTypeEncoding(Class cls, SEL sel) {
  ...
  char *encoding = method_getDescription(m)->types;
  @try {
...
- (void)collectMethods:(NSMutableDictionary *)methodMap
  ...
  CollectMethodsOrConstructors(self, methodMap, publicOnly, ^ id (SEL sel) {
    ...
    if (!IsConstructor(selStr)) {
      ...
      if (!HasSupportedTypeEncoding(self.objcClass, sel)) {
        return nil;
      }
