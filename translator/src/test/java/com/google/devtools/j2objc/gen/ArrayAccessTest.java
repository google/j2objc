...
public class ArrayAccessTest extends GenerationTest {
  ...
  public void testGetElement() {
    ...
    assertEquals("int one = [arr intAtIndex:0];", result);
    result = generateStatement(stmts.get(2));
    assertEquals("int two = [arr intAtIndex:1];", result);
  }
  ...
  public void testSetElementWithLiteral() {
    ...
    assertEquals("(*[arr intRefAtIndex:0]) = -1;", result);
    result = generateStatement(stmts.get(2));
    assertEquals("(*[arr intRefAtIndex:1]) = -2;", result);
  }
  ...
  public void testSetElementWithExpression() {
    ...
    assertEquals(3, stmts.size());
    String result = generateStatement(stmts.get(1));
    assertEquals("(*[arr intRefAtIndex:0]) = 2 * 5;", result);
    result = generateStatement(stmts.get(2));
    assertEquals("(*[arr intRefAtIndex:1]) = 6 / 3;", result);
  }
  ...
  public void testPrefixOperator() {
    ...
    assertEquals(3, stmts.size());
    String result = generateStatement(stmts.get(1));
    assertEquals("++(*[arr intRefAtIndex:0]);", result);
    result = generateStatement(stmts.get(2));
    assertEquals("--(*[arr intRefAtIndex:1]);", result);
  }
