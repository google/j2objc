...
public abstract class TypeGenerator extends AbstractSourceGenerator {
  ...
  protected String getMethodSignature(MethodDeclaration m, boolean asObjCGenericDecl) {
    ...
    String returnType =
        asObjCGenericDecl
            ? nameTable.getObjCGenericType(element.getReturnType())
            : nameTable.getObjCType(element.getReturnType());
    ...
    if (params.isEmpty()) {
      ...
      for (int i = 0; i < params.size(); i++) {
        ...
        String typeName =
            asObjCGenericDecl
                ? nameTable.getObjCGenericType(var.asType())
                : nameTable.getObjCType(var.asType());
        sb.append(
            UnicodeUtils.format(
                "%s:(%s%s)%s",
