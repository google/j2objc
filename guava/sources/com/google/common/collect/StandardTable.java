...
class StandardTable<R, C, V> implements Table<R, C, V>, Serializable {
  ...
  class Row extends AbstractMap<C, V> {
    ...
    public Set<C> keySet() {
      ...
      if (result == null) {
        @WeakOuter
        class RowKeySet extends Maps.KeySet<C, V> {
          @Override
          Map<C, V> map() {
            return Row.this;
          }
        }
        return new RowKeySet();
      }
      return result;
    }
      ...
      @Override protected Entry<R, V> computeNext() {
        while (iterator.hasNext()) {
          ...
          if (entry.getValue().containsKey(columnKey)) {
            @WeakOuter
            class EntrySetIteratorMapEntry extends AbstractMapEntry<R, V> {
            ...
            return new EntrySetIteratorMapEntry();
          }
        }
        return endOfData();
      }
